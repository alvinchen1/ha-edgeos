...
| {Router Name} Firmware Update        | Update        | Reports installed/latest firmware and installs the device-reported firmware | The firmware binary sensor remains available alongside a standard admin-only Update entity. Existing installations retain the binary sensor and receive the new update entity. Release notes and announcements link to the Ubiquiti Community releases page because Ubiquiti makes finding the exact EdgeOS release announcement unnecessarily difficult and does not provide a reliable release URL through the API. The router can reboot or disconnect after accepting an update, so completion is confirmed after a refresh. |
